<<<<<<< HEAD
cmp = {
	path:'../application/views/compras/',
	request:'../Compras/',		
	init:function(i){		
		
		$("body").catalogo({
			title : 'Compras',id_c:"cmp_c",view : cmp.request,post : i,
			callback : function(i) {			
				$("#tbl-cmp").scrollTable({
					parent : $("#catalogo-cmp"),
					source : cmp.request+ "comprasTable",
					extend : i,
					singleFilter : $("#busqueda-cmp"),
					advancedFilter: $("#srch-cmp")					
				});	
			}
		});
	},
	clean:function(i){$("#tbl-cmp").scrollTable('clean')},			
	nuevaCompra:function(o){
		cmp.producto = {},cmp.productos = {},cmp.productosDS = {},cmp.productosOC = {},cmp.orden={};

		$("body").formModal({title : "Nueva Compra",id : "cmp",modal :cmp.request+"nuevaCompra",post : o,
			callback : function(i) {
				md = $('#cmp-modal');				
				if(o && o.id_compra){				
				...
				rls = {factura:{remote:{ url: cmp.request+"facturaUnica",type: "POST",data: {factura: function() {return $.trim(md.find( "#factura" ).val())}}}}};
	...
	guardarCompra:function(o){		
		app.spin('#cmp-modal button.ladda-button');	
		...
		o.iva = cmp.iva;
		o.total = cmp.total;			
		$.ajax({type : "POST",url :cmp.request+ "guardarCompra",dataType : "json",data : o})
		.done(function(r) {1 == r.status ? (app.ok(),app.close('cmp'),app.updateByTag('cmp')) : app.error();})
		        	...
		        	$.confirm({ title: 'Borrar',content: 'Al borrar, se perderá toda la información relacionada con la compra, ¿Continuar?',
					    ...
					    buttons: {
					        ...
					        b: {text: 'Borrar',btnClass: 'btn-red', action: function(r){ 
					        	$.ajax({type : "POST",url : cmp.request+"borrarCompra",dataType : "json",data : o})
								.done(function(r) {
