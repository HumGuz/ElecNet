...
vnt = {
	...
	nuevaVenta:function(o){
		...
		callback : function(i) {
			...
			md.find("#nvaVnt").validation({extend:o,success:function(ob){
				var l = Object.keys(vnt.productos).length;
				if(!l){
					...
					// 	var p = vnt.productos[i];
					// 	var s = (  p.existencia < p.cantidad && p.um <! 'SERV' ? s + 1 : s );
					// 	var c = (  p.precio < p.costo_promedio ? c + 1 : c );
					// }					
					// if(s > 0){
	...
	totalGeneral:function(){
    	var dsc = $.trim($("#descuento_general").val());    	
    	if(dsc ==''){
